...
class DefaultStudyQueryAppServiceTest {
    ...
    void 모든_스터디_목록_조회_성공() {
        ...
        IntStream.range(0, allStudies.size())
                .forEach(index -> {
                    assertThat(allStudies.get(index).getLeader()).isEqualTo(studyList.get(index).getLeader().getName());
                    assertThat(allStudies.get(index).getName()).isEqualTo(studyList.get(index).getName());
                    assertThat(allStudies.get(index).getId()).isEqualTo(studyList.get(index).getId());
                });
    }
